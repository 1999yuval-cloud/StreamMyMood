...
MODEL_FILE     = "streammymood_model_v8.pkl"
...
def get_recommendations(answers, content_df, model_data, group_valid_titles, seen_ids=None):
    ...
    awards_required  = awards_matter
    reviews_required = review_val == "חשובות מאוד!"
    ...
    def in_group(c):
        return c["Title"] in valid_for_group
    ...
    # Pass 2 — relax runtime, keep group + filters
    if len(results) < 4:
    ...
    # Pass 3 — relax filters, keep group
    if len(results) < 4:
...
def screen_quiz():
    ...
    st.markdown(f'<div class="center" style="font-size:0.85rem;color:rgba(255,255,255,0.5);margin-bottom:0.8rem">שאלה {q_idx+1} מתוך {total}</div>',
                unsafe_allow_html=True)
    st.markdown(f'<div class="center" style="font-size:1.8rem;font-weight:800;margin-bottom:1.4rem;text-align:center">{q["text"]}</div>',
                unsafe_allow_html=True)
...
def screen_loading():
    ...
    for i in range(len(LOADING_TEXTS)):
        ph.markdown(f'<div class="reel-wrap">{REEL_SVG}<div class="loading-txt">{LOADING_TEXTS[i]}</div></div>',
                    unsafe_allow_html=True)
        time.sleep(0.75)
...
def screen_results(content_df, model_data, group_valid_titles):
    ...
    for col,r in zip(cols,batch):
        ...
        with col:
            ...
            if already_liked:
                st.markdown('<div style="text-align:center;color:#ff9ab0;font-size:1rem;margin-top:0.3rem">❤️ נוסף לאימון!</div>',
                            unsafe_allow_html=True)
            else:
