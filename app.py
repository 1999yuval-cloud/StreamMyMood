# ============================================================
#  StreamMyMood — app.py  v12 + Content_ID
# ============================================================
...
st.set_page_config(page_title="StreamMyMood", page_icon="🎬", layout="wide", initial_sidebar_state="collapsed")
...
def logo_html(width=680):
    if LOGO_B64:
        return (f'<div style="text-align:center;margin-bottom:0.5rem"><img src="data:image/png;base64,{LOGO_B64}" width="{width}" style="max-width:92vw;display:block;margin:0 auto"/></div>')
    return '<div style="text-align:center;font-size:2.5rem;font-weight:900;color:#ff4d6d">StreamMyMood</div>'

def placeholder_src():
    if LOGO_B64: return f"data:image/png;base64,{LOGO_B64}"
...
html, body, * {{ font-family: 'Assistant', Arial, sans-serif !important; direction: rtl !important; box-sizing: border-box; }}
.stApp, [data-testid="stAppViewContainer"] {{ {bg_css} background-color: #2d0012 !important; min-height: 100vh; color: white !important; }}
[data-testid="stAppViewContainer"]::before {{ content: ''; position: fixed; inset: 0; background: rgba(15,0,6,0.75); pointer-events: none; z-index: 0; }}
[data-testid="stVerticalBlock"] > div {{ display: flex !important; flex-direction: column !important; align-items: center !important; width: 100% !important; }}
div[data-testid="stRadio"], div[data-testid="stRadio"] > div {{ width: 100% !important; display: flex !important; flex-direction: column !important; align-items: center !important; }}
[data-testid="stHeader"] {{ background: transparent !important; }}
section[data-testid="stSidebar"] {{ display: none !important; }}
#MainMenu, footer, header {{ visibility: hidden; }}
.block-container {{ padding-top: 1.5rem !important; max-width: 1150px !important; margin: 0 auto !important; position: relative; z-index: 1; }}
...
div[data-testid="stRadio"] label[data-checked="true"] {{ background: rgba(180,0,50,0.5) !important; border-color: rgba(255,140,160,0.9) !important; font-weight: 800 !important; }}
.prog-outer {{ display: flex; justify-content: center; width: 100%; margin: 0.3rem 0 1.8rem 0; }}
.prog-wrap {{ background: rgba(255,255,255,0.12); border-radius: 10px; height: 6px; width: 560px; max-width: 88vw; }}
.prog-fill {{ background: linear-gradient(90deg, #b30030, #ff4d6d); height: 6px; border-radius: 10px; transition: width 0.4s ease; box-shadow: 0 0 8px rgba(255,77,109,0.5); }}
.movie-card {{ background: rgba(10,0,5,0.65); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 1rem; backdrop-filter: blur(8px); transition: transform 0.2s, box-shadow 0.2s; text-align: right; direction: rtl; height: 100%; }}
.movie-card:hover {{ transform: translateY(-4px); box-shadow: 0 8px 28px rgba(180,0,50,0.35); }}
.movie-poster {{ width: 100%; border-radius: 10px; margin-bottom: 0.7rem; object-fit: cover; height: 280px; background: #1a0008; display: block; }}
.movie-title {{ font-size: 1rem !important; font-weight: 800 !important; margin-bottom: 0.2rem !important; line-height: 1.3 !important; }}
.movie-meta {{ font-size: 0.8rem !important; color: rgba(255,255,255,0.6) !important; margin-bottom: 0.25rem !important; }}
.movie-platform {{ font-size: 0.8rem !important; color: #ff9ab0 !important; font-weight: 700 !important; margin-bottom: 0.3rem !important; }}
.movie-stars {{ color: #FFD700; font-size: 0.88rem; margin-bottom: 0.4rem; }}
.movie-overview {{ font-size: 0.82rem !important; color: rgba(255,255,255,0.8) !important; height: 100px; overflow-y: auto; line-height: 1.55; }}
.reel-wrap {{ text-align: center; padding: 3rem 0; }}
.film-icon {{ font-size: 5rem; display: inline-block; animation: movieRoll 1.5s ease-in-out infinite alternate; filter: drop-shadow(0 0 14px rgba(220,0,60,0.7)); }}
@keyframes movieRoll {{ 0% {{ transform: translateX(-30px) rotate(-10deg); }} 100% {{ transform: translateX(30px) rotate(10deg); }} }}
.loading-txt {{ font-size: 1.2rem !important; color: rgba(255,255,255,0.9) !important; margin-top: 1.4rem !important; font-weight: 600 !important; }}
...
MODEL_FILE    = "streammymood_model_v9.pkl"
FEEDBACK_FILE = "feedback_training.csv"

RUNTIME_RANGES = {
    "עד 30 דקות": (0,30), "30–60 דקות": (30,60),
    "60–120 דקות": (60,120), "מעל שעתיים": (60,9999),
}

WATCHING_MAP = {"לבד":"לבד","בן / בת זוג":"בדייט","משפחה":"משפחה","חברים":"חברים","ילדים":"ילדים"}
...
AWARDS_MAP = {"לא קריטי עבורי":"לא","כן, רק זוכי פרסים":"כן"}

QUESTIONS = [
    {"id":"time_available","text":"כמה זמן פנוי יש לך לצפייה?","options":["עד 30 דקות","30–60 דקות","60–120 דקות","מעל שעתיים"]},
    {"id":"watching_with","text":"עם מי את/ה נמצא?","options":["לבד","בן / בת זוג","משפחה","חברים","ילדים"]},
    {"id":"mood","text":"מה המצב רוח שלך כרגע?","options":[
        "קליל ומחפש/ת משהו שיעשה לי טוב על הלב","משועמם ומחפש/ת משהו שיעורר אותי",
        "סקרן ומחפש/ת סיפור להישאב אליו","רגיש ומחפש/ת משהו שייגע בי",
        "עמוס ומחפש/ת פשוט 'להיעלם' בתוך עולם אחר","מרוקן ורוצה להעביר את הזמן בלי לחשוב",
    ]},
    {"id":"review_importance","text":"עד כמה חשובות לך ביקורות?","options":["לא באמת משנה לי","חשובות מאוד!"]},
    {"id":"awards_preference","text":"האם חשוב שהתוכן זכה בפרסים?","options":["לא קריטי עבורי","כן, רק זוכי פרסים"]},
]

LOADING_TEXTS = ["מכינים המלצה שתתאים לך בול…","בודקים את מצב הרוח שלך…","סורקים את התכנים…","מחשבים התאמה אישית במיוחד בשבילך…"]
...
def load_data():
    ...
    content["Major_Awards_Won"] = pd.to_numeric(content["Major_Awards_Won"], errors="coerce").fillna(0)
    content["ID"] = content["ID"].astype(str)

    train = pd.read_excel(TRAINING_FILE)
    col_map = {"mood":"User_Mood","time_available":"Duration_Limit","watching_with":"Watch_Group",
               "awards_preference":"Award_Importance","rating_influence":"Review_Importance","chosen_content":"Content_Title"}
    ...
    if "Content_ID" in train.columns:
        ...
        group_valid_titles = {}
        for group in train["Watch_Group"].unique():
            ids = train[train["Watch_Group"]==group]["Content_ID"].tolist()
            titles = set(id_to_title.get(str(cid),"") for cid in ids)
            titles.discard("")
            group_valid_titles[group] = titles
    else:
        group_valid_titles = {}
        for group in train["Watch_Group"].unique():
            group_valid_titles[group] = set(train[train["Watch_Group"]==group]["Content_Title"].tolist())
...
def load_or_train(_train_df, _content_df):
    ...
    if os.path.exists(MODEL_FILE):
        try:
            if os.path.getmtime(TRAINING_FILE) <= os.path.getmtime(MODEL_FILE):
                needs_retrain = False
        except: pass
    if not needs_retrain: return joblib.load(MODEL_FILE)
...
def train_model(train_df, content_df):
    ...
    clf.fit(X_tr, y_tr)
    data = {"model":clf,"encoders":encoders,"le_target":le_target,"features":features,"accuracy":clf.score(X_val,y_val)}
...
def save_feedback(answers, content_title):
    ...
    row = {
        ...
        "chosen_content": content_title, "content_type": "",
...
def get_recommendations(answers, content_df, model_data, group_valid_titles, seen_ids=None):
    ...
    watch_display = answers.get("watching_with","")
    watch_group = WATCHING_MAP.get(watch_display, watch_display)
    ...
    if review_val == "חשובות מאוד!": w_model, w_rating, w_awards = 0.50, 0.40, 0.10
    elif awards_matter: w_model, w_rating, w_awards = 0.55, 0.25, 0.20
    else: w_model, w_rating, w_awards = 1.00, 0.00, 0.00

    answer_map = {
        "Duration_Limit": time_choice, "Watch_Group": watch_group,
        "User_Mood": MOOD_MAP.get(mood, mood),
        "Review_Importance": REVIEW_MAP.get(review_val, "לא אכפת לי מדירוגים"),
    ...
    valid_for_group = group_valid_titles.get(watch_group, set())

    def passes_filters(c):
        if awards_matter:
            if float(c.get("Major_Awards_Won",0) or 0) <= 0: return False
    ...
    if len(results) < 4:
        ...
        results+=extras2
        if extras2: st.caption("לא מצאנו מספיק תכנים עם כל הדרישות שלך — הוספנו תכנים קרובים.")
...
def card_html(c):
    ...
    if not poster or poster in ["nan","None",""]: poster=placeholder_src()
    ph=placeholder_src()
    ...
    return f"""<div class="movie-card"><img class="movie-poster" src="{poster}" alt="{title}" onerror="this.onerror=null;this.src='{ph}'"/><div class="movie-title">{title}</div><div class="movie-meta">{year} · {runtime}</div>{plat}<div class="movie-stars">{stars_str} <span style="color:rgba(255,255,255,0.55);font-size:0.78rem">{rat_str}</span></div><div class="movie-overview">{overview}</div></div>"""

def screen_welcome():
    st.markdown(logo_html(680), unsafe_allow_html=True)
    tz=pytz.timezone('Asia/Jerusalem')
    h=datetime.now(tz).hour
    g=("בוקר טוב" if h<12 else "צהריים טובים" if h<17 else "ערב טוב" if h<21 else "לילה טוב")
    st.markdown(f'<div class="center" style="margin-bottom:0.3rem;margin-top:0"><span style="font-size:2.2rem;font-weight:900;letter-spacing:-0.5px">{g}!</span></div><div class="center" style="margin-bottom:1.8rem"><span style="font-size:1.25rem;color:rgba(255,255,255,0.82);line-height:1.6">ברוכים הבאים ל‑Stream My Mood<br>בואו נמצא לכם מה לראות.</span></div>', unsafe_allow_html=True)
...
def screen_quiz():
    ...
    st.markdown(f'<div class="prog-outer"><div class="prog-wrap"><div class="prog-fill" style="width:{pct}%"></div></div></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="center" style="font-size:0.85rem;color:rgba(255,255,255,0.5);margin-bottom:0.8rem">שאלה {q_idx+1} מתוך {total}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="center" style="font-size:1.8rem;font-weight:800;margin-bottom:1.4rem;text-align:center">{q["text"]}</div>', unsafe_allow_html=True)
    cur=st.session_state.answers.get(q["id"])
    idx=q["options"].index(cur) if cur in q["options"] else 0
    sel=st.radio("בחרו תשובה", q["options"], index=idx, key=f"r_{q_idx}", label_visibility="hidden")
    ...
    with c2:
        ...
        if st.button(lbl):
            if q_idx<total-1: st.session_state.q_index+=1
            else: st.session_state.screen="loading"
            st.rerun()
...
def screen_loading():
    ...
    for i in range(len(LOADING_TEXTS)):
        ph.markdown(f'<div class="reel-wrap">{REEL_SVG}<div class="loading-txt">{LOADING_TEXTS[i]}</div></div>', unsafe_allow_html=True)
        time.sleep(0.75)
...
def screen_results(content_df, model_data, group_valid_titles):
    ...
    st.markdown('<div class="center" style="font-size:1.25rem;font-weight:800;margin-bottom:1.4rem">ההמלצות שלנו עבורך</div>', unsafe_allow_html=True)
