...
/* Kill ALL automatic Streamlit spacing */
.block-container > div {{
  gap: 0 !important;
  padding: 0 !important;
}}
...
MODEL_FILE     = "streammymood_model_v6.pkl"
...
RUNTIME_RANGES = {
    ...
    "30–60 דקות":  (30,  60),
    "60–120 דקות": (60, 120),
    "מעל שעתיים":  (60, 9999),  # has 2+ hrs = can watch anything 60min+
}
...
def load_data():
    ...
    mood_titles = train.groupby("User_Mood")["Content_Title"].apply(set).to_dict() if "User_Mood" in train.columns else {}
    group_valid_titles = train.groupby("Watch_Group")["Content_Title"].apply(set).to_dict() if "Watch_Group" in train.columns else {}
    return content, train, group_valid_titles

# ── MODEL ─────────────────────────────────────────────────────
@st.cache_resource
def load_or_train(_train_df, _content_df):
    # Check if model needs retraining (training data newer than model)
    needs_retrain = True
    ...
    # Train once and cache — this runs only when needed
...
def save_feedback(answers, content_title):
    # Save training values — MUST match training DB column values exactly
    mood_display    = answers.get("mood","")
    watch_display   = answers.get("watching_with","")
    review_display  = answers.get("review_importance","")
    awards_display  = answers.get("awards_preference","")
    ...
    row = {
        ...
        "content_type":      "",
        "mood":              MOOD_MAP.get(mood_display, mood_display),
        "time_available":    answers.get("time_available",""),
        "watching_with":     WATCHING_MAP.get(watch_display, watch_display),
        "awards_preference": AWARDS_MAP.get(awards_display, "לא"),
        "rating_influence":  REVIEW_MAP.get(review_display, "לא אכפת לי מדירוגים"),
    }
...
def get_recommendations(answers, content_df, model_data, group_valid_titles, seen_ids=None):
    ...
    awards_val  = answers.get("awards_preference","")

    if review_val == "חשובות מאוד!" and awards_val == "כן, רק זוכי פרסים":
        w_model, w_rating, w_awards = 0.40, 0.35, 0.25
    elif review_val == "חשובות מאוד!":
        w_model, w_rating, w_awards = 0.50, 0.45, 0.05
    elif awards_val == "כן, רק זוכי פרסים":
        w_model, w_rating, w_awards = 0.60, 0.10, 0.30
    else:
        # לא אכפת מדירוג ולא מפרסים — המודל לבד מחליט
        w_model, w_rating, w_awards = 1.00, 0.00, 0.00
    ...
    # Build valid titles for this watch group from training data
    watch_group_mapped = WATCHING_MAP.get(answers.get("watching_with",""), answers.get("watching_with",""))
    ...
    # Filter flags from content DB (not model weights)
    ...
    def passes_filters(c):
        ...
        if awards_required:
            ...
            if float(aw) <= 0:
                return False
        # Rating filter — hard, use content DB rating
        if reviews_required:
            r = c.get("Rating")
            if r is None or pd.isna(r) or float(r) < 8.0:
                return False
        return True

    def is_valid_for_group(c):
        """Only show content that appears in training data for this watch group"""
        return c["Title"] in group_valid_titles.get(watch_group_mapped, set())

    # Pass 1: runtime + awards/reviews + group filters
    results = []
    for _,c in content_df.iterrows():
        ...
        if not is_valid_for_group(c): continue
        results.append({"row":c,"score":score(c),"id":cid})
    results.sort(key=lambda x:x["score"],reverse=True)

    # Pass 2: relax runtime, keep hard filters
    if len(results) < 4:
...
def screen_welcome():
    ...
    st.markdown(f"""
    <div class="center" style="margin-bottom:0.3rem;margin-top:0">
...
def main():
    ...
    # group_valid_titles from load_data
