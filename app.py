...
def get_recommendations(answers, content_df, model_data, seen_ids=None):
    ...
    # Minimum threshold — model must be confident enough
    # titles with top probability get score, others are blocked
    top_ms = max(model_scores.values()) if model_scores else 1.0
    ms_threshold = top_ms * 0.05  # must be at least 5% of top score

    def score(c):
        ms = model_scores.get(c["Title"], 0.0)
        # Block titles where model has very low confidence
        if ms < ms_threshold:
            return 0.0
        r  = c.get("Rating")
    ...
    # Pass 1: runtime + awards/reviews filters
    results = []
    ...
    # Pass 3: relax runtime but keep awards/reviews filters
    if len(results) < 4:
    ...
    # Pass 4: last resort — relax everything except mood-based model score
    if len(results) < 4:
        seen4 = {r["id"] for r in results} | seen_ids
        extras3 = []
        for _, c in content_df.iterrows():
            cid = str(c["ID"])
            if cid in seen4: continue
            extras3.append({"row":c,"score":score(c),"id":cid})
        extras3.sort(key=lambda x: x["score"], reverse=True)
        results += extras3
