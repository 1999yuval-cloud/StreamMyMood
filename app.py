...
def get_recommendations(answers, content_df, model_data, seen_ids=None):
    ...
    def score(c):
        ms = model_scores.get(c["Title"], 0.01)  # small non-zero for unseen titles
        r  = c.get("Rating")
    ...
    # Pass 3: if still <4, relax ALL filters (runtime + awards + reviews)
    # This prevents empty results screen
    if len(results) < 4:
